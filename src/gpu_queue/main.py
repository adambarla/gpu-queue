...
# Try importing requests, but it might not be installed in system python?
# The user environment seems to use `uv run`, so dependencies should be there if installed.
# If not, we can fall back or user needs to install.
try:
    import requests
except ImportError:
    requests = None
...
def get_available_gpu_indices() -> list[int]:
    """Get indices of GPUs that are currently free (excluding those assigned to running jobs)."""
    gpus = get_free_gpus()
...
def is_daemon_running() -> bool:
    """Check if the scheduler is potentially running. 
    Since we removed PID files, this is just a placeholder or could check process list."""
        ...
        try:
            ...
            with locked_queue() as queue:
                if queue["pending"]:
                    ...
                    try:
                        status_data = {"ts": datetime.now().isoformat(), "gpus": all_gpus}
...
def cmd_serve(args):
    ...
    if getattr(args, "exclude_gpus", None):
        try:
            ...
            print(f"Error: Invalid format for --exclude-gpus.")
            sys.exit(1)
...
def cmd_start(args):
    ...
    # Write PID file with configuration (store min_free_gpus too maybe? simpler to just pass arg)
...
class GPUQueueTUI:
    def __init__(self, interval=2.0):
        ...
        self.modal = None # { type: "CONFIRM"|"INPUT", title, text, value, on_confirm, on_cancel }
                    ...
                    try:
                        ...
                        if txt.strip():
                            ...
                            with self.lock:
                             ...
                             with self.lock: self.server_status = "DAEMON: S?"
                    except json.JSONDecodeError:
                        pass # Partial write?
                else:
                    with self.lock: self.server_status = "DAEMON: OFF"
            except Exception:
                 with self.lock:
    ...
    def draw_box(self, y, x, h, w, title, active=False, focused=False):
        ...
        try:
            ...
            self.stdscr.box()
            # self.stdscr.rectangle(y, x, y+h-1, x+w-1) # rectangle doesn't use relative coords well with subwin?
            ...
            # Title
            title_str = f" {title} "
            if focused: title_str = f" [ {title} ] "
            elif active: title_str = f" {title} "
            
            self.stdscr.addstr(y, x+2, title_str, color | (curses.A_BOLD if active else 0))
    ...
    def format_job_line(self, job, w):
        ...
        # Determine Color based on state/status (user request: windows and color do the job)
        color = curses.color_pair(0)
        
        prefix = ""
        
        if job["_type"] == "running":
            # Formt: ID(8) | GPUS(8) | ELAPSED(9) | CMD
            ...
            prefix = f" {jid:<8} {gpus:<8} {elapsed:<9} "
            
        elif job["_type"] == "pending":
            # Format: ID(8) | PRIO(6) | WAITING(9) | CMD
            # color default (white/inactive or orange/active via window, but item itself usually plain or bold?)
            ...
            if s_res == "success": color = curses.color_pair(2) # Green
            elif s_res == "failed": color = curses.color_pair(1) # Red
            elif s_res == "cancelled": color = curses.color_pair(3) # Orange
        ...
        if len(cmd) > avail_cmd: cmd = cmd[:(avail_cmd-1)] + "…"
    ...
    def draw(self):
        ...
        with self.lock:
            ...
            server_col = curses.color_pair(2) if "ON" in self.server_status else curses.color_pair(1)
            self.stdscr.addstr(0, 1, " GPU QUEUE WATCH ", status_col | curses.A_BOLD)
            self.stdscr.addstr(0, 20, f" [{self.server_status}] ", server_col | curses.A_BOLD)
            ...
            if self.action_msg:
                if time.time() > self.msg_clear_time:
                    ...
                    self.stdscr.addstr(0, msg_x, f" {self.action_msg} ", curses.color_pair(3) | curses.A_REVERSE)
            ...
            for i, win in enumerate(self.windows):
                wh = heights[i]
                if wh <= 0: continue # Skip hidden windows
    ...
    def draw_modal(self, h, w):
        ...
        try:
            ...
            # Title
            self.stdscr.addstr(y, x + 2, f" {self.modal['title']} ", curses.color_pair(3) | curses.A_BOLD)
            ...
            if self.modal["type"] == "INPUT":
                ...
                # Import textwrap if not imported globally, but let's assume valid import or use simple slicing
                ...
                # If cursor is at the exact end matching a multiple of width, we need an empty line cursor?
                # Actually if len(val) % field_w == 0 and len(val)>0, cursor at end is new line
                # But our split logic puts it at end of last line if we use slice.
                ...
                for i in range(field_h):
                    ...
                    # Only draw if valid line or active cursor line (which might be empty at end)
                    if line_idx * field_w <= len(val):
                    ...
                    if line_idx == c_row:
                        ...
                        # Wait, if cursor_pos == len(val), and len(val) is multiple of width,
                        # c_row = len // width, c_col = 0.
                        # So it draws on next line.
                        
                        char_at = " "
                        if c_col < len(line_content):
                             char_at = line_content[c_col]
                             
                        self.stdscr.addstr(draw_y, x+3+c_col, char_at, curses.A_REVERSE)
    ...
    def draw_compact_gpu_info(self, y, x, h, w):
        ...
        try:
            ...
            # Column headers
            header = "IDX  MEM_USAGE      PROCESSES (USER:PID)"
            if w > 30:
                self.stdscr.addstr(y, x+2, header[:w-4], curses.A_DIM | curses.A_UNDERLINE)
            ...
            for i, g in enumerate(self.gpu_status[:h-1]):
                ...
                # Make status text gray regardless of state (only header shows status via color)
                ...
                line_y = y + 1 + i
                self.stdscr.addstr(line_y, x+2, f"{idx:<4} {mem_s:<15}", color)
                
                procs = g.get("processes", [])
                proc_strs = [f"{p.get('user','?')}:{p.get('pid','?')}" for p in procs if not p.get("zombie")]
    ...
    def draw_job_details(self, y, x, h, w):
        ...
        try:
            ...
            for i, line in enumerate(lines[:h-1]):
                self.stdscr.addstr(y+1+i, x+2, prefix if i==0 else " "*len(prefix), curses.A_DIM)
    ...
    def draw_window(self, win, y, x, h, w, active, focused):
        ...
        try:
            # Border Color logic
            if focused: color = curses.color_pair(5) # Blue
            elif active: color = curses.color_pair(6) # White
            else: color = curses.color_pair(7) # Gray
            ...
            # Title
            title_s = f" {win.title} "
            if win.collapsed: title_s = f" [+] {win.title} "
            elif focused: title_s = f" [ {win.title} ] "
            ...
            for i, item in enumerate(visible_items):
                ...
                if is_sel:
                    if focused:
                        ...
                        # In NAV mode, we don't reverse the item, we just highlight the window
                        pass
    ...
    def draw_footer(self, y, w):
        try:
            ...
            help_str = " Q:Quit "
            if self.mode == "NAV":
                help_str += "Arrows/j/k:Select Window  Enter:Focus  [:Collapse  ]:Uncollapse"
            else:
                help_str += "Esc:Back  Space/l:Log  c:Cancel  +/-:Prio  g:Gpus  r:Remove  d:Dup  n:New  p:Pause"
    ...
    def draw_log_overlay(self, h, w):
        ...
        try:
            ...
            self.stdscr.addch(margin_y+win_h-1, margin_x+win_w-1, curses.ACS_LRCORNER)
            self.stdscr.attroff(curses.color_pair(3))
            # Title
            title = f" LOGS: {self.log_job_id} "
            self.stdscr.addstr(margin_y, margin_x + 2, title, curses.A_BOLD | curses.A_REVERSE)
            
            # Content
            content_h = win_h - 2
            visible_lines = self.log_content[self.log_scroll : self.log_scroll + content_h]
            
            for i, line in enumerate(visible_lines):
                if len(line) > win_w - 2: line = line[:win_w-5] + "..."
                self.stdscr.addstr(margin_y + 1 + i, margin_x + 1, line, curses.A_NORMAL)
                
            # Footer
            footer_str = " Esc/l:Close  Arrows:Scroll  PGUP/PGDN:Jump  L:Open in Fullscreen (less) "
            self.stdscr.addstr(margin_y + win_h - 1, margin_x + 2, footer_str, curses.A_REVERSE)
            
        except: pass

    def action_view_logs(self):
        if self.mode != "ACTION": return
        ...
        if log_path.exists():
            try:
                with open(log_path) as f:
                    self.log_content = [l.rstrip() for l in f.readlines()]
            except:
    ...
    def main(self, stdscr):
        ...
        curses.init_pair(7, 8, -1) # Gray (if color 8 exists, else fallback happens or it's ignored)
        # Fallback for gray if 8 is not available
        if curses.COLORS > 8:
            pass
        else:
             curses.init_pair(7, curses.COLOR_WHITE, -1) # Fallback to white but usually use A_DIM
        ...
        try:
            while True:
                ...
                if self.modal:
                    ...
                    if m["type"] == "CONFIRM":
                         if ch == ord('y') or ch == 10: # Yes
                             if m["on_confirm"]: m["on_confirm"]()
                             self.modal = None
                         elif ch == ord('n') or ch == 27: # No / Esc
                             if m["on_cancel"]: m["on_cancel"]()
                             self.modal = None
                         ...
                         if ch == 27: # Esc
                             ...
                             if m["on_confirm"]: m["on_confirm"](val)
                             self.modal = None
                ...
                # Log Viewing Mode
                if self.viewing_logs:
                    if ch == 27 or ch == ord('l') or ch == ord('q') or ch == ord(' '): # Esc or l or q
                        self.viewing_logs = False
                    elif ch == curses.KEY_UP:
                        self.log_scroll = max(0, self.log_scroll - 1)
                    elif ch == curses.KEY_DOWN:
                        max_scroll = max(0, len(self.log_content) - (self.stdscr.getmaxyx()[0] - 6))
                        self.log_scroll = min(max_scroll, self.log_scroll + 1)
                    elif ch == curses.KEY_PPAGE:
                        self.log_scroll = max(0, self.log_scroll - (self.stdscr.getmaxyx()[0] - 6))
                    elif ch == curses.KEY_NPAGE:
                        max_scroll = max(0, len(self.log_content) - (self.stdscr.getmaxyx()[0] - 6))
                        self.log_scroll = min(max_scroll, self.log_scroll + (self.stdscr.getmaxyx()[0] - 6))
                    continue
                ...
                if self.mode == "NAV":
                    if ch == curses.KEY_UP or ch == ord('k'):
                        ...
                        self.active_win_idx = min(len(self.windows) - 1, self.active_win_idx + 1)
                    elif ch == 10: # Enter
    ...
    def prompt_new_job(self):
        self.modal = {
            ...
            "value": "",
            "cursor_pos": 0,
            "on_confirm": lambda val: self.add_job_internal(val, 2, 1), # Default 2 GPUs, Med Prio
            "on_cancel": None
        }
    ...
    def prompt_change_gpus(self):
        ...
        self.modal = {
            ...
            "on_confirm": lambda val: self.execute_action("change_gpus", job['id'], new_val=val),
            "on_cancel": None
        }
    ...
    def do_action(self, action):
        ...
        if action == "cancel":
             ...
             self.modal = {
                 ...
                 "on_confirm": lambda val: self.add_job_internal(val, job.get("gpus", 2), 1),
                 "on_cancel": None
             }
             return
    ...
    def execute_action(self, action, jid, **kwargs):
        ...
        try:
            if action == "cancel":
                ...
                subprocess.Popen(["gpu-queue", "cancel", jid], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
                msg = f"Cancelling {jid}..."
                 ...
                 subprocess.Popen(["gpu-queue", "pause", jid], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
                 msg = f"Pausing {jid}..."
...
def main():
    ...
    # start
    start_parser = subparsers.add_parser("start", help="Start the queue scheduler (background)")
